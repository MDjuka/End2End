...
from end_to_end.utilities.utils import generate_random_email_and_password
...
class CheckoutPage:
    ...
    def input_billing_fname(self, first_name="TestFname"):
        self.base.wait_and_input_text(self.BILLING_FIRST_NAME_FIELD, first_name)

    def input_billing_lname(self, lname="TestLname"):
        self.base.wait_and_input_text(self.BILLING_LAST_NAME_FIELD, lname)

    def input_billing_street_address(self, address="123 Main St"):
        self.base.wait_and_input_text(self.BILLING_STREET_ADDRESS_FIELD, address)

    def input_billing_town(self, town="San Francisco"):
        self.base.wait_and_input_text(self.BILLING_TOWN_FIELD, town)

    def input_billing_zip(self, zip_code="94018"):
        self.base.wait_and_input_text(self.BILLING_ZIP_FIELD, zip_code)

    def input_billing_phone_num(self, phone_num="8051111111"):
        self.base.wait_and_input_text(self.BILLING_PHONE_FIELD, phone_num)

    def input_billing_email(self, email=None):
        if not email:
            rand_email = generate_random_email_and_password()
            email = rand_email['email']
        self.base.wait_and_input_text(self.BILLING_EMAIL_FIELD, email)
    ...
    def click_place_order_btn(self):
        self.base.wait_and_click(self.PLACE_ORDER_BTN)
